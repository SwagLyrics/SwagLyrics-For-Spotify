...
class Tests(unittest.TestCase):
    ...
    def test_that_unsupported_precheck_initial_check_handles_permission_error(self, f_time):
        ...
        self.assertIn(
            "You should install SwagLyrics as --user or use sudo to access unsupported.txt.", capturedOutput.getvalue()
        )
    ...
    @patch(
        ...
        return_value=argparse.Namespace(tab=True, cli=False, no_issue=False, update_check=False),
    )
    ...
    @patch(
        "argparse.ArgumentParser.parse_args",
        return_value=argparse.Namespace(tab=False, cli=True, no_issue=True, update_check=False),
    )
