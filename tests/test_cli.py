...
class Tests(unittest.TestCase):
	...
	def test_that_stripping_works(self):
		...
		self.assertEqual(stripper('Ain\'t My Fault - R3hab Remix',
								  'Zara Larsson'), 'Zara-Larsson-Aint-My-Fault')
	...
	def test_that_get_lyrics_does_not_break_with_wrong_data(self):
		...
		self.assertEqual(get_lyrics("Faded", "Muhmello", False),
						 "Couldn't get lyrics for Faded by Muhmello.\n")
		self.assertEqual(get_lyrics("Battle Symphony", "Drake", False),
						 "Couldn't get lyrics for Battle Symphony by Drake.\n")

		# Deleting above songs and artists from unsupported.txt
		with open(config.get_unsupported_path(), "r") as f:
			lines = f.readlines()
		with open(config.get_unsupported_path(), "w") as f:
			for line in lines:
	...
	def test_that_lyrics_works_for_unsupported_songs(self):
		...
		self.assertEqual(lyrics("Fantastic", "Beasts"),
						 "Lyrics unavailable for Fantastic by Beasts.\n")
	...
	def test_that_lyrics_do_not_break_with_file_not_found(self):
		...
		"""
		os.rename(config.get_unsupported_path(), "unsupported2.txt")
		self.assertEqual(lyrics("Crimes", "Grindelwald", False),
						 "Couldn\'t get lyrics for Crimes by Grindelwald.\n")
	...
	def test_that_get_lyrics_calls_requests(self, mock_requests):
		...
		"""
		self.assertEqual(get_lyrics("River", "Dale", True),
						 "Couldn't get lyrics for River by Dale.\nSeason 3 is supernatural")
